...
2. **Driver Implementations**: Platform-specific drivers. Implementations live in `mobilerun-core` (cloud) and `mobilerun-core-local` (local/on-device); the `mobilerun/tools/driver/*` modules below are stable compat import paths that re-export them.
   - `AndroidDriver`: Android devices via ADB + Portal app (implementation in `mobilerun-core-local`)
   - `IOSDriver` (compat path `tools/driver/ios.py`): iOS devices via HTTP REST API to Portal app (implementation in `mobilerun-core-local`)
   - `StealthDriver` (compat path `tools/driver/stealth.py`): Wraps another driver, adds human-like timing jitter (implementation in `mobilerun-core-local`)
   - `RecordingDriver` (compat path `tools/driver/recording.py`): Wraps another driver with trajectory recording (implementation in `mobilerun-core-local`)
   - `CloudDriver` (compat path `tools/driver/cloud.py`): Cloud-hosted device driver (implementation in `mobilerun-core`)
...
<a id="mobilerun.tools.ui.provider.StateProvider"></a>
...
<a id="mobilerun.tools.ui.state.UIState"></a>
...
<a id="mobilerun.agent.action_context.ActionContext"></a>
...
<a id="mobilerun.agent.action_result.ActionResult"></a>
...
<a id="mobilerun.agent.tool_registry.ToolRegistry"></a>
...
| press_button | back, home, enter | home only (back/enter raise ValueError) |
...
| get_apps | Full packages with labels | Not supported for device enumeration; use known bundle IDs with `start_app()` |
...
- [AndroidDriver API](/sdk/adb-tools) - Android driver implementation
- [IOSDriver API](/sdk/ios-tools) - iOS driver implementation
- [MobileAgent API](/sdk/droid-agent) - Agent integration
- [Configuration](/sdk/configuration) - Configuration reference
