...
Output format: include `<thought>`, then exactly one non-empty result tag: either a
`<plan>` while work remains, or a final `<request_accomplished>` with an explicit
`success="true"` or `success="false"` attribute. Never include both result tags.
...
Or if complete (do not include `<plan>`):
<thought>Your reasoning</thought>
<request_accomplished success="true">
Task is done. Answer: ...
</request_accomplished>

Use `success="false"` with an explanation instead when the task cannot be completed.
"""
}
...
Return `<thought>` followed by exactly one non-empty result tag: use `<plan>`
while work remains, or `<request_accomplished success="true|false">` when the
task is complete or blocked. Never include both `<plan>` and a final result tag.
"""
}
...
Output: include `<thought>`, then exactly one non-empty result tag.
...
Or when the task is complete or blocked (do not include `<plan>`):
<request_accomplished success="true|false">
Explain the completed result or why the task could not be completed.
</request_accomplished>
"""
}
