...
ManagerConfig(
    vision=False,                 # Enable screenshots
    system_prompt=None,           # Auto-select the bundled prompt for the selected manager mode
    stateless=False,              # Use StatelessManagerAgent (no conversation history)
)
```

`manager.system_prompt: null` (or `None` in Python) selects the bundled prompt
automatically: `config/prompts/manager/system.jinja2` for the stateful manager and
`config/prompts/manager/stateless.jinja2` for the stateless manager. Set an explicit
path to pin a template instead. A runtime `prompts={"manager_system": "..."}`
template has precedence over either configured choice.

Config migration v7 changes the former canonical stateful default path to `null` so
that `stateless: true` receives the stateless prompt automatically. If you had
intentionally pinned that exact canonical path, add it back after migration; other
...
agent:
  ...
  manager:
    vision: false
    system_prompt: null  # auto-selects stateful or stateless bundled prompt
    stateless: false
